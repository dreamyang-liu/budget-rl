...
def _make_target(remaining_tokens: int, target_mode: str, interval_type: str,
    ...
    if reasoning:
        if probe_after == 0:
            return gt, (
                f"<think>Given the initial grid and typical Sokoban trajectories, "
...
def _process_trajectory_impl(traj, tokenizer, max_tokens, probe_every_n,
    ...
    task_success = metadata.get("CoordSokoban/success", 0.0) == 1.0
    ...
    for probe_after in probe_positions:
        ...
        sft_messages = history + [
            {"role": "user", "content": probe_content},
    ...
    with open(args.input) as fin:
        for line in fin:
            ...
            for probe in probes:
                ...
                rl_records.append({
                    "data_source": "budget_probe_sokoban",
