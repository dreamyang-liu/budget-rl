...
class ReActAgent(Agent):
    ...
    REASONING_REGEX = re.compile(r"^\s*Reasoning:\s*(.+)Action:", re.M | re.S) # Everything up till Action
    ACTION_REGEX = re.compile(r"^\s*Action:\s*(.+)", re.M)
    ...
    def propose_actions(self, obs, env):
        ...
        while len(matching_action) == 0 and feedback_steps < self.max_feedback_steps:
            ...
            # Log prompt/response but only add to history after validation.
            ...
            # Update history only after valid reasoning/action
