...
def _build_vllm_prompt(messages, user_prompt, history=None):
    """Build a plain-text prompt for vLLM from chat messages."""
    prompt_parts = []
...
def _get_vllm_engine(model, **kwargs):
    """Get or create a cached vLLM engine for the given model and params."""
    cache_key = (model, tuple(sorted(kwargs.items())))
    engine = VLLM_ENGINES.get(cache_key)
    if engine is None:
        engine = LLM(model=model, **kwargs)
        VLLM_ENGINES[cache_key] = engine
    return engine
...
def prompt_llm(user_prompt, messages, model, temperature, history=[], **kwargs):
    ...
    if debug:
        ...
        vllm_params = {
            "dtype": kwargs.get("vllm_dtype", "auto"),
            ...
            "enforce_eager": kwargs.get("vllm_enforce_eager", False),
            "max_model_len": kwargs.get("vllm_max_model_len", kwargs.get("max_length", 8192)),
            "enable_prefix_caching": kwargs.get("vllm_enable_prefix_caching", True),
            "trust_remote_code": kwargs.get("vllm_trust_remote_code", True),
        }
        llm = _get_vllm_engine(model, **vllm_params)
        prompt = _build_vllm_prompt(messages, user_prompt, history=history)
        logger.debug("[VLLM INPUT PROMPT]\n%s", prompt)
        sampling_kwargs = {
            "temperature": temperature,
            "max_tokens": kwargs.get("vllm_max_tokens", 512),
        }
        top_p = kwargs.get("vllm_top_p", None)
        top_k = kwargs.get("vllm_top_k", None)
        if top_p is not None:
            sampling_kwargs["top_p"] = top_p
        if top_k is not None:
            sampling_kwargs["top_k"] = top_k
        outputs = llm.generate([prompt], sampling_params=SamplingParams(**sampling_kwargs))
        response = outputs[0].outputs[0].text
        logger.debug("[VLLM OUTPUT TEXT]\n%s", response)
