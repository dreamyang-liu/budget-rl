...
def run_robotouille(environment_name: str, agent_name: str, **kwargs: Dict[str, Any]):
    ...
    # Initialize agent
    llm_kwargs = dict(kwargs.get('llm_kwargs', {}))
    log_only_success = llm_kwargs.pop("log_only_success", False)
    final_log_path = None
    temp_log_path = None
    if log_only_success and llm_kwargs.get("log_path"):
        final_log_path = llm_kwargs["log_path"]
        temp_log_path = final_log_path + ".tmp"
        llm_kwargs["log_path"] = temp_log_path
    ...
    if log_only_success and final_log_path and temp_log_path and os.path.exists(temp_log_path):
        if done:
            os.makedirs(os.path.dirname(final_log_path), exist_ok=True)
            if os.path.exists(final_log_path):
                with open(final_log_path, "a") as f_out, open(temp_log_path, "r") as f_in:
