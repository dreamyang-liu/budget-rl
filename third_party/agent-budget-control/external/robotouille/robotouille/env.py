...
def build_input_json(domain_json):
    ...
    if not os.path.isabs(input_json_name):
        input_json_name = os.path.normpath(os.path.join(os.path.dirname(__file__),"..", input_json_name))
...
class RobotouilleEnv(gym.Env):
    ...
    def step(self, action):
        if isinstance(action, str):
            valid_actions, str_valid_actions = self.current_state.get_valid_actions_and_str()
            if action not in str_valid_actions:
                raise ValueError(f"Invalid action string: {action}")
            action = [valid_actions[str_valid_actions.index(action)]]
        ...
        return obs, reward, done, {"success": bool(done)}
