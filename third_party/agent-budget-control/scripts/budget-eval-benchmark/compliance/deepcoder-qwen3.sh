...
RUN_NAME=${RUN_NAME:-deepcoder_qwen3_eval_compliance}
...
OUTPUT_DIR=${OUTPUT_DIR:-"$RESULT_ROOT/deepcoder-compliance-token-qwen3-32b-instant-512-main"}
...
cmd=(
  ...
  "agent_proxy.eval_compliance_token_scope=[2000,3000,4000,5000,6000]"
...
echo "==> Running DeepCoder compliance with local Qwen3-32B thinking model at ${MODEL_PATH}"
if [[ "$DRY_RUN" == "1" ]]; then
  printf '%q ' "${cmd[@]}"
  printf '\n'
  exit 0
