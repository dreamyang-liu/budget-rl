...
RUN_NAME=${RUN_NAME:-sokoban_qwen3_eval_compliance}
...
OUTPUT_DIR=${OUTPUT_DIR:-"$RESULT_ROOT/sokoban-compliance-turn-qwen3-32b-instant-512-test"}
...
echo "==> Running Sokoban compliance with local Qwen3-32B thinking model at ${MODEL_PATH}"
if [[ "$DRY_RUN" == "1" ]]; then
  printf '%q ' "${cmd[@]}"
  printf '\n'
  exit 0
