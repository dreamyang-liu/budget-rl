...
RUN_NAME=${RUN_NAME:-webshop_trained_eval_adaptation}
...
ADAPTATION_BUDGET_AFTER=${ADAPTATION_BUDGET_AFTER:-3}
RESULT_ROOT=${RESULT_ROOT:-"$PWD/results/budget-estimation-benchmark"}
OUTPUT_DIR=${OUTPUT_DIR:-"$RESULT_ROOT/webshop-adaptation-turn-${MODEL_TAG}"}
...
echo "==> Running WebShop turn-level adaptation with local merged model at ${MODEL_PATH}"
if [[ "$DRY_RUN" == "1" ]]; then
  printf '%q ' "${cmd[@]}"
  printf '\n'
  exit 0
