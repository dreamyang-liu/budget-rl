#!/bin/bash
# Robotouille mixed toolcall-budget RL runner.
# This script enables mixed toolcall budget training and is intended for
# single-node launches such as 4xH200 sbatch jobs.
...
MIXED_TOOLCALL_RANGE=""
MIXED_TOOLCALL_RANGE_EXPLICIT=0

RUN_NAME=""
LOGGER="['console','wandb']"
OUTPUT_ROOT="logs/training"
CHECKPOINT_ROOT="/projects/bflz/model_saving/robotouille-mixed-toolcall"
PREFLIGHT_ONLY="${PREFLIGHT_ONLY:-0}"

usage() {
    cat <<'EOF'
Usage: bash scripts/training/robotouille/mixed-budget-training-robotouille-mixed-toolcall.sh [options]

Robotouille mixed toolcall-budget RL:
  - mixed toolcall budget: enabled
  - mixed toolcall budget range: [10, 20]
  - mixed turn budget: disabled
  - mixed token budget: disabled
  - reward curve: action points used vs sampled action-point budget
...
Options:
  ...
  --max-action-points N          Robotouille action budget cap / mixed range upper bound. Default: 25
  ...
  --mixed-toolcall-range A,B     mixed toolcall budget range. Default: 10,20
  --checkpoint-root PATH         Checkpoint root. Default: /projects/bflz/model_saving/robotouille-mixed-toolcall
...
budget_range_tag() {
    echo "${MIXED_TOOLCALL_RANGE//,/to}"
}

build_run_name() {
    echo "robotouille-mixed-toolcall-${ALGO}-${GPUS_PER_EXP}gpu-$(model_tag)-${NUM_GROUPS}x${GROUP_SIZE}-turn${MAX_TURN}-budget$(budget_range_tag)-mixed"
}
...
normalize_range() {
    ...
    if [ -z "${low:-}" ] || [ -z "${high:-}" ] || [ -n "${extra:-}" ]; then
        echo "Error: mixed toolcall range must be two integers like 4,25." >&2
        exit 1
    fi
    if [[ ! "$low" =~ ^-?[0-9]+$ ]] || [[ ! "$high" =~ ^-?[0-9]+$ ]]; then
        echo "Error: mixed toolcall range must contain integers, got '$1'." >&2
        exit 1
    fi
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --mixed-toolcall-range) MIXED_TOOLCALL_RANGE=$(normalize_range "$2"); MIXED_TOOLCALL_RANGE_EXPLICIT=1; shift 2 ;;
        --mixed-toolcall-range=*) MIXED_TOOLCALL_RANGE=$(normalize_range "${1#*=}"); MIXED_TOOLCALL_RANGE_EXPLICIT=1; shift ;;
...
if [ "$MIXED_TOOLCALL_RANGE_EXPLICIT" -ne 1 ]; then
    MIXED_TOOLCALL_RANGE="10,20"
fi
MIXED_TOOLCALL_RANGE=$(normalize_range "$MIXED_TOOLCALL_RANGE")
resolve_gpu_selection

IFS=',' read -r MIXED_TOOLCALL_LOW MIXED_TOOLCALL_HIGH <<< "$MIXED_TOOLCALL_RANGE"
IFS=',' read -r -a GPU_IDS <<< "$GPUS"
...
COMMON_OVERRIDES=(
    ...
    "agent_proxy.mixed_toolcall_budget.enabled=True"
    "agent_proxy.mixed_toolcall_budget.mixed_budget=True"
    "agent_proxy.mixed_toolcall_budget.mixed_budget_range=[${MIXED_TOOLCALL_RANGE}]"
    "agent_proxy.mixed_toolcall_budget.reward_curve.use_hard=False"
    "agent_proxy.mixed_toolcall_budget.reward_curve.tau=1.0"
...
echo "Mode: mixed toolcall budget enabled, mixed turn budget disabled, mixed token budget disabled"
echo "Robotouille env: ${ENV_NAME}"
echo "Context window mode: ${CONTEXT_WINDOW_MODE}"
echo "Max context window: ${MAX_CONTEXT_WINDOW}"
echo "Action budget cap: ${MAX_ACTION_POINTS}"
echo "Mixed toolcall range: [${MIXED_TOOLCALL_RANGE}]"
echo "Algorithm: $ALGO"
echo "GPUs: $GPUS"
